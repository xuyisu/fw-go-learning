...
func main() {
	...
	{
		v2.POST("/", createSysUser)      //POST方法，创建新用户
		v2.GET("/", fetchAllSysUser)     //GET方法，获取所有用户
		v2.GET("/:id", fetchSysUser)     //GET方法，获取某一个用户，形如：/api/v2/user/1
		v2.PUT("/:id", updateSysUser)    //PUT方法，更新用户，形如：/api/v2/user/1
		v2.DELETE("/:id", deleteSysUser) //DELETE方法，删除用户，形如：/api/v2/user/1
	}
	router.Run(":8081")
}
...
// 获取所有用户
func fetchAllSysUser(c *gin.Context) {
...
// 获取单个用户
func fetchSysUser(c *gin.Context) {
	var user SysUser      //定义SysUser结构体
	data := c.Param("id") //获取参数id

	db.First(&user, data)

	if user.Id == 0 { //如果用户不存在，则返回
		c.JSON(http.StatusNotFound,
			gin.H{"code": http.StatusNotFound, "msg": "用户不存在!"})
		return
	}
	//返回响应结构体
	res := SysUserRes{
...
func deleteSysUser(c *gin.Context) {
	...
	db.Delete(&user)
	c.JSON(http.StatusOK,
		gin.H{"code": http.StatusOK, "msg": "用户删除成功!"})
}
